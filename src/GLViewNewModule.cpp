...
void GLViewNewModule::updateWorld()
{
   ...
   scene->fetchResults(true);
   {
...

	   {
		   ...
		   Module5WOP* wo = static_cast<Module5WOP*>(actor->userData);
...
void GLViewNewModule::createSphere()
{
	float rx, ry = 0;
	srand(r);
	rx = rand() % 5;
	...
	Module5WOP* wo = Module5WOP::New(p, scene, (ManagerEnvironmentConfiguration::getSMM() + "/models/dice_twelveside_outline.wrl"), { 1, 1, 1 }, Aftr::MESH_SHADING_TYPE::mstAUTO, false, PxVec3{ rx, ry, 25 });
...
void GLViewNewModule::createFloor()
{
...
void Aftr::GLViewNewModule::loadMap()
{
	...
	//https://github.com/NVIDIAGameWorks/PhysX/blob/4.1/physx/snippets/snippetstepper/SnippetStepper.cpp
   ...
   wo = Module5WOP::New(p, scene, grass, Vector(1, 1, 1), MESH_SHADING_TYPE::mstFLAT, true, PxVec3{ 0, 0, 0 });
   wo->setPosition( Vector(0,0,0) );
   wo->renderOrderType = RENDER_ORDER_TYPE::roOPAQUE;
   wo->getModel()->getModelDataShared()->getModelMeshes().at(0)->getSkins().at(0).getMultiTextureSet().at(0)->setTextureRepeats( 5.0f );
   wo->setLabel( "Grass" );
